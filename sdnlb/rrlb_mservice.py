...
class rrlb(DynamicPolicy):
	...
	#	dst_01 = match(srcip=pkt['srcip'], dstip=IPAddr('10.0.0.2'))
		...
		if pkt['dstport'] in self.ports:		
			dsts = dict()
			i = 1
			port = pkt['dstport']
			dst_srv = match(srcip=pkt['srcip'], dstip=IPAddr('10.0.0.2'),dstport=port, ethtype=packet.IPV4, protocol=packet.TCP_PROTO)
			...
			self.policy = if_(other_switches, identity, \
							if_(dst_srv, modify(dstip=self.ip,dstmac=self.mac), \
